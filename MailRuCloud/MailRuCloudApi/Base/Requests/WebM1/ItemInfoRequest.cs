...
namespace YaR.MailRuCloud.Api.Base.Requests.WebM1	
{
    class ItemInfoRequest : BaseRequestJson<FolderInfoResult>
    {
        ...
        public ItemInfoRequest(IWebProxy proxy, IAuth auth, string path, bool isWebLink = false, int offset = 0, int limit = int.MaxValue) 
            : base(proxy, auth)
        {
            _path = path;
        ...
        protected override string RelationalUri
        {
            ...
            {
                var uri = _isWebLink
                    ? $"/api/m1/file?access_token={Auth.AccessToken}&weblink={Uri.EscapeDataString(_path)}&offset={_offset}&limit={_limit}"
